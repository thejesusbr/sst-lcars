...
let lcarsAudio = {
	...
	initialize: function(auDummy) {
		...
		this.audTactInputAcknowledge = new Audio("audio/computerbeep_17." + this.audioType);
		...
		button_list = document.querySelectorAll("#container div.lcars-elbow.button, div.lcars-app-container div.lcars-elbow.button");
		for (var button of button_list) {
			console.log("Audio-enabling container-contained button " + button.id + " " + button.innerText);
			button.addEventListener("click", function(){ lcarsAudio.TactileInputAcknowledge(); }); 
		}
		...
		for (var button of button_list) {
			console.log("Audio-enabling button " + button.id + " " + button.querySelector('#label')	);
			button.addEventListener("click", function(){ lcarsAudio.TactileInputAcknowledge(); }); 
		}
...
//=\\ Standard Wire-up
let audDummy = document.getElementById("audDummy").appendChild(document.createElement("audio"));
