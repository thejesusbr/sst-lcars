...
- [x] 1.1 `types/game.ts` — tipo `TurnEvent`: etapa de origem (1-5), tipo de
...
- [x] 2.1 `engine/constants.ts` — LUT `WARP_ANIMATION_MS` decrescente
      ...
      (design.md decisão 7)
- [x] 2.2 `engine/turnEngine.ts` — engajar warp **esvazia `currentSector`** no
...
- [x] 2.3 `engine/turnEngine.ts` — rejeitar **toda** ação que consome turno
      enquanto `warpTrip` existe, com motivo. Ações livres seguem passando
- [x] 2.4 `stores/useGameState.ts` — modo de viagem: ao engajar, avança os turnos
      restantes sozinho até a chegada, respeitando a duração da LUT entre eles
- [x] 2.5 `components/modules/HelmConsole.vue` — LUT substitui o piso fixo de 5s
      (`WARP_MIN_VISUAL_MS`), e a duração passa a ser por turno de viagem
- [x] 2.6 Testes: viagem multi-turno resolve sem input; ação recusada em trânsito
...
- [x] 3.1 `stores/useGameState.ts` — fila dos eventos do turno + flag
      `presenting`. Um dono só do timer (design.md decisão 2), limpo ao
      desmontar/trocar de tela
- [x] 3.2 `stores/useGameState.ts` — turno novo é recusado enquanto a fila drena;
      a fila não é sobrescrita no meio
- [x] 3.3 Encenar SÓ combate. Reparo de CdD, regen de pool e avanço de stardate
      aplicam direto (design.md decisão 8)
- [x] 3.4 Testes de store: fila drena na ordem; ação recusada durante drenagem;
      nenhum timer sobrevive ao fim

## 4. Encenação na UI (depende da Fase 3)

- [x] 4.1 `LcarsScanner` — camada de **overlay transitório**, separada do
      ...
      (design.md decisão 9)
- [x] 4.2 Animação de phaser: linha pulsante entre quem atira e o alvo. Vale
      pro jogador E pro inimigo — ver o inimigo agir é o ponto da mudança
- [x] 4.3 Animação de torpedo: asterisco percorrendo as células até o alvo
- [x] 4.4 Feedback de absorção de escudo / dano em casco, em sequência
- [x] 4.5 Degradação do display por DANO no sensor (não por confiança — essa
      segue só esmaecendo). Moderado (`d > 0.30`): SRS/LRS piscam. Crítico
      (`d > 0.60`): display apaga por completo (design.md decisão 8)
- [x] 4.5b LRS em moderado ou pior: dígitos do KBS **variam aleatoriamente** na
      ...
      o display dança
- [x] 4.6 Todos os consoles desabilitam ação que consome turno lendo a MESMA flag
      da store (`presenting` ou viagem em curso) — nenhum decide por conta
- [x] 4.7 Verificar que um turno tranquilo (sem inimigo) **não** tem espera:
...
- [x] 5.2 `npx vitest run --project unit` verde, com os testes das fases 2 e 3
- [x] 5.3 Confirmar que o engine segue **síncrono e sem `setTimeout`**: o grep de
