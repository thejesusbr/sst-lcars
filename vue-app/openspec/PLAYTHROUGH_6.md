...
| 13.6 3 inimigos = escudo zerado no turno 1 | **Aplicado.** `ENEMY_ATTACKERS_PER_TURN=1` — revezamento por índice rotativo (`stardate % ativos`) entre hostis do setor; só quem está na vez tenta atacar, os demais recarregam. DPS de longo prazo igual, pico simultâneo eliminado | `combat-pressure` |
| Obs. geral: destruída no turno 1 | **Aplicado.** `pickStartPosition` agora reamostra o QUADRANTE (não só a célula) até `content.klingons === 0` | `combat-pressure` |
| 30.1 Carregar tubo é armadilha | **Aplicado.** `requestTubeLoad` — ação livre (não custa turno), marca `loadPending`; `resolvePendingTubeLoads` completa no fim da resolução em curso, pronto pro turno seguinte (mesmo padrão de `dispatchTeam`). Só descarregar continua custando turno | `combat-pressure` |
